            ...
            <li>
              <router-link to='/'>
                <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                  width="40px" height="40px" viewBox="0 0 63.699 63.699" style="enable-background:new 0 0 63.699 63.699;"
                  xml:space="preserve">
                <g>
                  <path d="M63.663,29.424c-0.143-1.093-0.701-2.065-1.575-2.737l-11.715-9.021V8.608c0-2.275-1.851-4.126-4.125-4.126
                    ...
                    l13.205,10.166c0.084,0.064,0.108,0.147,0.116,0.205C59.891,29.975,59.885,30.062,59.819,30.144z" fill="#fff"/>
                </g></svg>
            </router-link>
            </li>
            <li v-for="(item, index) in getMenu" :key="index" class="menu__item">
              <router-link :to="{name: 'Inner', params: { id: item.id }}" class="menu__link">
                {{ item.name }}
              </router-link>
              <ul class="menu__child">
                <li v-for="(child, key) in item.childs" :key="key" >
                  <router-link :to="{name: 'Inner', params: { id: child.id }}" class="menu__link">
                    {{ child.name }}
                  </router-link>
                </li>
              </ul>
...
export default {
  ...
  methods: {
    goToRoute(id) {
      this.$router.push({ name: 'Inner', params: { id } })
    }
  }
}
</script>
